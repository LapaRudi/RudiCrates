...
public class MySQL {
    ...
    public void connect() {
        try {
            if (isConnected() && !getConnection().isClosed()) {
                Bukkit.getConsoleSender().sendMessage(Messages.PREFIX + "§eEine MySQL Verbindung wurde bereits aufgebaut.");
                return;
            }
            setConnection(DriverManager.getConnection("jdbc:mysql://" + host + ":" + port + "/" + database, username, password));
            Bukkit.getConsoleSender().sendMessage(Messages.PREFIX + "§aMySQL Verbindung wurde aufgebaut.");

        } catch (SQLException e) {
            Bukkit.getConsoleSender().sendMessage(Messages.PREFIX + "§cMySQL Verbindung konnte nicht hergestellt werden. Überprüfe ob die Datenbank online ist, und die Angaben in der Config richtig sind.");
            Bukkit.getConsoleSender().sendMessage(Messages.PREFIX + "§4RudiCrates wird deaktiviert, wenn du MySQL nicht nutzen willst, setze 'usemysql' in der Config auf false.");
    ...
    private String crateAmount() {
        int size = CrateUtils.getCrates().length;
        if(size == 1) return "0";
